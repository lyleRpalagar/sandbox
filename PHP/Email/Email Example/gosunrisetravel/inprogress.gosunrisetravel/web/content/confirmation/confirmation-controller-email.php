...
// ******************************************************************************************
 ...
 // WHATEVER variable you created up top must be added down below in the same syntax 
    ...
    $email_message .= "Arrival: ".clean_string($arrive)."\n";
...
//copy the traffic of the email to the mailcopyfile.txt (NEED to be DELETED AFTER TESTING)
//create a file and name it mailcopyfile.txt this is where all the data will be stored 
$mailcopyfile='../confirmation/mailcopyfile.txt';
$fp = fopen($mailcopyfile, "a"); 
fputs($fp, $now . $email_to . $email_subject . $email_message . $headers . "\n" ); //variables may change depending on what you are tracking
fclose($fp);
